  ...
  (setq-default
   ...
   dotspacemacs-configuration-layers '(
     ...
     (javascript :variables
                 ...
                 js2-basic-offset 2
                 ;; Adds support for local eslint installations
     ...
     (version-control :variables
                      ...
                      ;; TODO try both of these and see which one feels better
...
before packages are loaded."
  ...
  ;; Configure vim style undo/redo
  (global-undo-tree-mode)
  (evil-set-undo-system 'undo-tree)

  ;; xml-mode is causing memory leaks on mac so disable it
  (add-to-list 'auto-mode-alist '("\\.xsl\\'" . html-mode))

  ;; Run eslint --fix on the current file after save
  ...
  ;; TODO put org-journal stuff into its own layer
  ;; org-journal default template
  (defun org-custom/new-buffer-p ()
    (not (file-exists-p (buffer-file-name))))

  (defun org-custom/insert-journal-template ()
    (let ((template-file (expand-file-name "journal/journal-template.org" org-directory)))
      (when (org-custom/new-buffer-p)
        (save-excursion
          (goto-char (point-min))
  ...
  (setq org-capture-templates
    '(("t" "TODO" plain (file+headline "~/org/todo.org" "TODAY") "** TODO %?\n  %i\n")
...
%i%?"
  ...
  ;; Custom agenda view to display daily log with columns for exporting
  (setq org-agenda-custom-commands
    '(("d" "Day agenda with columns" tags "DATE={.+}"
        ((org-agenda-overriding-header "Daily Log")
         (org-agenda-overriding-columns-format "%DATE %ITEM %ENTRY_TYPE %JIRA_PROJECT %JIRA_TICKET_NUM")
         (org-agenda-view-columns-initially t)))))

  (defun export-daily-log ()
    "Updates the the dynamic block for the daily log and exports it to csv."
    (interactive nil)
    (org-dblock-update)
    (org-set-property "TABLE_EXPORT_FORMAT" "orgtbl-to-csv")
    (org-set-property "TABLE_EXPORT_FILE" (concat "csv/in/" (file-name-sans-extension (buffer-name)) ".csv"))
    (forward-line 1)
    (org-table-export))

  (defun export-daily-log-with-csv ()
    "Updates the the dynamic block for the daily log and exports it to csv."
    (interactive nil)
    (org-dblock-update)
    (org-set-property "TABLE_EXPORT_FORMAT" "orgtbl-to-csv")
  ...
  ;; ruby code folding - does not seem to work?
